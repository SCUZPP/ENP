...
def initialization(pop_size, gene_length, model, args, train_loader, test_loader, val_loader, p_a, p_b, target_dir):
    ...
    for i in range(len(p_a)):
        ...
        with open(path_save + "population_300_2.pkl", 'wb') as f:
                pickle.dump(population, f) 
       
        with open(path_save + "population_30_2.pkl", 'wb') as f:
...
def get_population(model, args, train_loader, test_loader, val_loader):
    ...
    open_path_a = 'population_dict/population_dict_a_vgg%s.pkl' % (args.depth)
    open_path_b = 'population_dict/population_dict_b_vgg%s.pkl' % (args.depth)
    p_a = []
    ...
    target_dir = 'Results2_vgg%s/' % (args.depth)
