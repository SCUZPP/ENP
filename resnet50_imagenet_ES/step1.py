...
def evaluate(ind, model, args, train_loader, test_loader, val_loader, g):
    ...
    acc_full = 23.76
    if np.sum(solution) == sum(filter_nums):
        return acc_full, np.sum(solution) 
    ...
    if len(device_ids)>1:
        #print('parallel')
        model_new = torch.nn.DataParallel(model_new).cuda();#前    
    '''
    # Validate
    #只剪枝不微调
    acc, loss = test_forward(val_loader, model_new, args)
    #acc, loss, temp = train_forward(train_loader, test_loader, val_loader, model_new, args)
...
def fitness(pop_a, pop_b, pop_c, g, pivot, model, args, train_loader, test_loader, val_loader):
    #pop_a = copy.deepcopy(population_a)
    #pop_b = copy.deepcopy(population_b)
    
    filter_nums = args.filter_nums
    gene_nums_a = args.gene_a
    sample_time = args.sample_time
    error_rates_a = []
    

    used_prob = [[] for i in range(len(gene_nums_a))]
    used_indexs = [[] for i in range(len(gene_nums_a))]
    
    #store the fitness value of every ind in population B
    score_pop_b = [[] for i in range(len(gene_nums_a))]
    len_pop_b = [[] for i in range(len(gene_nums_a))]
    
    for i in range(len(gene_nums_a)):
    ...
    for ind_a in pop_a:
        ...
        for j in range(len(gene_nums_a)):
        ...
        for time in range(sample_time):
            solution = []
            #cfg = []
            # the first conv layer keeps unchanged
            ...
            for layer in range(len(ind_a.dec)):
                ...
                #print('prob', prob)
                #conv1 in residual block
                solution.append(pop_b[layer][prob][index].dec.tolist())
                #cfg.append(int(sum(pop_b[layer][prob][index].dec)))
                
                residual_index += 1
                #conv3 in residual block
                #solution.append(pop_b[layer][0][0].dec.tolist())
                if residual_index % 3 == 0:
                    length = len(pop_b[layer][prob][index].dec.tolist())
                    temp = [1] * length * 4               
                    solution.append(temp)
                    residual_index += 1
                
            #the last fully connected layer
            #temp = [1] * int(filter_nums[args.depth] - 1)
            ...
            #print('solution', solution)
            #print('pop_c[count_c].dec[:]', pop_c[count_c].dec.shape)
            pop_c[count_c].dec[:] = solution
            #print('time', time)
            #print('solution', solution)
            solution.reshape(sum(filter_nums), 1)
            #evalutae the pruned model decoded by solution
            error_rate, filter_re = evaluate(solution, model, args, train_loader, test_loader, val_loader, g)
    ...
    for r in range(len(pop_a)):
        for layer in range(len(gene_nums_a)):
...
def get_population(model, args, train_loader, test_loader, val_loader):
    ...
    dec_dim_b = gene_nums_a  # Decision variable dimensionality
    
 
    # Initialization population B
    population_b = [[] for j in range (len(gene_nums_a))]
    ...
    population_c = initialization_c(pop_size_c, filter_length)
    pivot =  pop_size_a * args.sample_time
    
    g_begin = 0
    target_dir_a = 'Results1_A_resnet%s/' % (args.depth)
    target_dir_b = 'Results1_B_resnet%s/' % (args.depth)
    target_dir_c = 'Results1_C_resnet%s/' % (args.depth)
    ...
    for g in range(g_begin + 1, gen + 1):
        ...
        pivot =  0
        used_probs = fitness(population_a, population_b, population_c, g, pivot, model, args, train_loader, test_loader, val_loader)
        offspring_b = copy.deepcopy(population_b)
        #Variation for population B
        #the last fully connected layer keeps unchanged
        for j in range (len(gene_nums_a) - 1):
        ...
        pivot =  pop_size_a * args.sample_time
        used_probs = fitness(population_a, offspring_b, population_c, g, pivot, model, args, train_loader, test_loader, val_loader)
        # P+Q
    ...
    #以文件形式存储
    save_path_a = 'population_dict/population_dict_a_resnet%s.pkl' % (args.depth)
                                                                                 
    pickle.dump(population_a, open(save_path_a, 'wb'))

    save_path_b = 'population_dict/population_dict_b_resnet%s.pkl' % (args.depth)
                                                                                 
    pickle.dump(population_b, open(save_path_b, 'wb'))
    
    save_path_c = 'population_dict/population_dict_c_resnet%s.pkl' % (args.depth)
                                                                                 
    pickle.dump(population_c, open(save_path_c, 'wb'))
